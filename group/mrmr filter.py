...
for i in range(500):
    for j in np.arange(i,500):
        MI_ff[i,j] = mutual_info_regression((breast_train_scaled_1.iloc[0:78,i]).reshape((78,1)),(breast_train_scaled_1.iloc[0:78,j]).reshape((78,1)), n_neighbors=5)  
        if i % 10 ==0:
           print(i)
...
MI_fl = mutual_info_regression(breast_train_scaled_1.iloc[0:78,:], label_train, n_neighbors=5)
...
breast_train_50 = np.zeros((79,50))
count = 0
for i in range(500):
    if candidate[i] == 1:
        breast_train_50[:,count] = breast_train_scaled_1.iloc[:,i]
        count += 1
